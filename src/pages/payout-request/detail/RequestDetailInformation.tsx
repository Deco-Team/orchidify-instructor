...
export const Field: React.FC<FieldProps> = ({ label, content, requestStatusTag, hasMadePayout }) => (
  <Box display='flex'>
    ...
    {hasMadePayout && (
      <>
        <Typography variant='subtitle1' fontWeight={400} mr={1}>
          Đã thanh toán
        </Typography>
        <Check color='success' />
      </>
        ...
        <Paper sx={{ display: 'flex', flexDirection: 'column', p: 3, gap: 2.5 }} elevation={2}>
          ...
          <ContentWrapper>
            {request.hasMadePayout ? (
              <>
                <Field label='Trạng thái' hasMadePayout={request.hasMadePayout} />
              ...
              <Field label='Trạng thái' content={'Chưa thanh toán'} />
            )}
          </ContentWrapper>
        </Paper>
      )}
