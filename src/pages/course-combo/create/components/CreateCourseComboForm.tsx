...
import { HeaderWrapper, Line, StyledForm } from './CreateCourseComboForm.styled'
...
interface CreateCourseComboFormProps {
  courses: CourseListItemResponseDto[]
}

const CreateCourseComboForm = ({ courses }: CreateCourseComboFormProps) => {
  const navigate = useNavigate()
  const { createCourseCombo } = useCourseComboApi()
  ...
  const onSubmit = handleSubmit(async (formData) => {
    ...
    const { data, error } = await createCourseCombo(formData)
    if (data) {
      notifySuccess(APP_MESSAGE.ACTION_DID_SUCCESSFULLY('Tạo Combo khóa học'))
      navigate(-1)
    }
      ...
      <Grid container columnSpacing={4} rowSpacing={'20px'}>
        ...
        <Grid item xs={6}>
          <Autocomplete
            size='small'
            disablePortal
            options={courses.filter((course) => !formValues.childCourseIds.includes(course._id))}
...
export default CreateCourseComboForm
