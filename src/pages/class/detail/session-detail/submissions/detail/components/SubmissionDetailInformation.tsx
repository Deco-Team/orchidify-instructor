    ...
    <Paper sx={{ display: 'flex', flexDirection: 'column', p: 3, gap: 2.5 }} elevation={2}>
      ...
      <Box display='flex' gap='1rem'>
        ...
        <Box display='flex' flexDirection='column' gap={1} flexGrow='1'>
          ...
          {submission.status !== SubmissionStatus.NOT_YET && (
            <>
              <Field label='Thời gian nộp' content={submission.createdAt?.toLocaleString('vi-vn')} />

              <Field label='Cập nhật cuối' content={submission.updatedAt?.toLocaleString('vi-vn')} />
            </>
          )}
      ...
      {submission.status !== SubmissionStatus.NOT_YET && (
        <>
          <Field label='Giảng viên' content={submission.examiner?.name} />
          ...
          <Box>
            ...
            >
              <div style={{ width: '100%', height: '100%', padding: '0 2px' }}>
                ...
                submission.attachment.resource_type === 'image' &&
                submission.attachment.format !== 'pdf' ? (
                  <img
                    src={submission.attachment.url}
                    alt={`Lesson resource ${submission.attachment.public_id}`}
                    style={{
                      width: '200px',
                      height: '200px',
                      objectFit: 'cover',
                    ...
                    onClick={() => submission.attachment && window.open(submission.attachment.url, '_blank')}
                  />
                ) : (
                  <Box
                    sx={{
                  ...
                  >
                    ...
                    {submission.attachment && (
                      ...
                      >
                        {submission.attachment.public_id}
                      </Typography>
                    )}
                  </Box>
                )}
