  ...
  genQuickLink({
    ...
    if (this.checkKey()) {
      let url =
        media == '.jpg'
          ? encodeURI(
              `${this.apiUrl}/${bank}/${accountNumber}/${amount}/${memo}/${template}.jpg?accountName=${accountName}`
            ).replace(/%20/g, '+')
          : encodeURI(
              `${this.apiUrl}/${bank}/${accountNumber}/${amount}/${memo}/${template}.png?accountName=${accountName}`
            ).replace(/%20/g, '+')
      return url
    }
    this.sendMessage(this.checkKey())
